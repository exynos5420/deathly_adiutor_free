...
public class CPU implements Constants {
    ...
    public static void activatePerCoreControl(boolean active, Context context) {
        ...
        if (active) {
            Control.deletespecificcommand(context, CPU_MSM_LIMITER_SCALING_GOVERNOR, null);
            for (int i = 0; i < CPU.getCoreCount(); i++) {
                CPU.setMSMLimiterGovernorPerCore(CPU.getMSMLimiterGovernor(-1), context, i);
            }
        }
        else {
            for (int i = 0; i < CPU.getCoreCount(); i++) {
            ...
            CPU.setMSMLimiterGovernor(CPU.getMSMLimiterGovernorPerCore(0), context, -1);
        }
    }
    ...
    public static void setMSMLimiterGovernor(String governor, Context context, int core) {
        if (getMSM_Limiter_Version() < 5.2 && core == -1) {
    ...
    public static void setMSMLimiterGovernorPerCore(String governor, Context context, int core) {
        if (getMSM_Limiter_Version() < 5.2) {
            ...
            setMSMLimiterGovernor(governor, context, core);
        }
    }
    ...
    // For now, the easiest way to work around this is to just call the previous function recursively for per core controls.
    public static String getMSMLimiterGovernorPerCore (int core) {
        ...
        else if (getMSM_Limiter_Version() > 5.1 && core >= 0) {
         return getMSMLimiterGovernor(core);
        }
        return "";
    }
